...
class PhotonSmartClusterPhotonInsightsParamsStruct(
  ...
  val minInterval: UInt,
  val maxInterval: UInt,
  val dropWifiLogs: Boolean,
  val reportMetrics: Boolean,
  val reportHeapMetrics: Boolean,
  val heapPollingInterval: UInt,
  val heapPollingCount: UByte,
  val reportWifiMetrics: Boolean,
  val wifiPollingInterval: UInt,
  val wifiPollingCount: UByte,
  ...
  companion object {
    ...
    fun fromTlv(tlvTag: Tag, tlvReader: TlvReader): PhotonSmartClusterPhotonInsightsParamsStruct {
      ...
      val minInterval = tlvReader.getUInt(ContextSpecificTag(TAG_MIN_INTERVAL))
      val maxInterval = tlvReader.getUInt(ContextSpecificTag(TAG_MAX_INTERVAL))
      ...
      val heapPollingInterval = tlvReader.getUInt(ContextSpecificTag(TAG_HEAP_POLLING_INTERVAL))
      val heapPollingCount = tlvReader.getUByte(ContextSpecificTag(TAG_HEAP_POLLING_COUNT))
      val reportWifiMetrics = tlvReader.getBoolean(ContextSpecificTag(TAG_REPORT_WIFI_METRICS))
      val wifiPollingInterval = tlvReader.getUInt(ContextSpecificTag(TAG_WIFI_POLLING_INTERVAL))
